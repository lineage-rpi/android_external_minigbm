...
uint32_t gbm_mesa_resolve_format(struct driver *drv, uint32_t format, uint64_t use_flags)
{
	switch (format) {
 ...
 */
static std::array<std::string, 4> separate_dc_gpu_list = {"lima", "panfrost", "v3d"};
...
static std::shared_ptr<GbmMesaDriver> gbm_mesa_get_or_init_driver(struct driver *drv, bool mapper_sphal)
{
	std::shared_ptr<GbmMesaDriver> gbm_mesa_drv;
	...
	if (!drv->priv) {
		...
		if (look_for_kms && !mapper_sphal) {
			...
			/* cardX KMS node need this otherwise composer won't be able to configure KMS state */
			if (!gbm_mesa_drv->gbm_node_fd)
				drmDropMaster(gbm_mesa_drv->gbm_node_fd.Get());
			else
				ALOGE("Failed to find/open /dev/card node with KMS capabilities.\n");
		} else {
			gbm_mesa_drv->gbm_node_fd =
				UniqueFd(dup(gbm_mesa_drv->gpu_node_fd.Get()));
		}
...
static int gbm_mesa_get_spoofed_format_data(uint32_t in_format, int in_height, uint32_t *out_format, int *out_height) {
	int first_plane_bpp = drv_bytes_per_pixel_from_format(in_format, 0);
	switch (first_plane_bpp) {
	case 1: *out_format = DRM_FORMAT_R8;
		break;
	case 2: *out_format = DRM_FORMAT_GR88;
		break;
	case 4: *out_format = DRM_FORMAT_ARGB8888;
		break;
	default:
	...
	*out_height = in_height;
	...
	for (size_t i = 1; i < drv_num_planes_from_format(in_format); i++) {
		...
		assert(plane_bpp != 0);
		*out_height += DIV_ROUND_UP(in_height, (first_plane_bpp / plane_bpp));
	}

	return 0;
}

int gbm_mesa_bo_create(struct bo *bo, uint32_t width, uint32_t height, uint32_t format, uint64_t use_flags)
{
	...
	if (!err)
	    err = gbm_mesa_alloc(drv->gbm_driver, width, s_height, s_format, scanout, linear, &single_plane_fd, &stride, &bo->meta.format_modifier);
...
int gbm_mesa_bo_import(struct bo *bo, struct drv_import_fd_data *data)
{
	...
	if (data->use_flags & BO_USE_SW_MASK) {
		...
		if (get_gbm_mesa_format(s_format) == 0) {
			gbm_mesa_get_spoofed_format_data(data->format, data->height, &s_format, &s_height);
		}

		priv->gbm_bo = gbm_import(drv->gbm_driver, data->fds[0], data->width, s_height, data->strides[0], data->format_modifier, s_format);
	}
...
{;
	...
	if (get_gbm_mesa_format(s_format) == 0) {
		gbm_mesa_get_spoofed_format_data(bo->meta.format, bo->meta.height, &s_format, &s_height);
	}
